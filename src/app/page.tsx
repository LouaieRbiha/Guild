...
const RECENT_ROASTS = [
	{
		...
		element: 'Hydro',
		verdict: 'YOUR BUILD IS SO BAD 🔥',
	},
	{
		uid: 'UID823yyy',
		character: 'Furina',
		element: 'Hydro',
		verdict: "CRIT RATE CAPPED? YOU'RE DOING IT WRONG! 💀",
	},
	{
		uid: 'UID901zzz',
		character: 'Kazuha',
		element: 'Anemo',
		verdict: 'ARTIFACTS LOOK LIKE TRASH 😂',
	},
];
			...
			<section className='pt-24 pb-12 px-6'>
				<div className='max-w-5xl mx-auto text-center space-y-8'>
					<h1 className='text-6xl font-bold tracking-tight'>
						<span className='text-guild-accent'>✦</span> GUILD{' '}
						<span className='text-guild-accent'>✦</span>
					</h1>
			...
			<section className='px-6 pb-12'>
				<div className='max-w-5xl mx-auto space-y-4'>
					...
					<div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4'>
						{(typeof QUICK_PICKS !== 'undefined' && QUICK_PICKS?.length
							? QUICK_PICKS
							: [
									{
										id: 'mock-1',
							...
							return (
								...
								>
									...
									<div className='absolute bottom-3 left-3 right-3'>
										...
										{Icon && <Icon size={14} className='inline ml-auto' />}
									</div>
				...
				<div className='max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4'>
					...
					<div className='guild-card p-5 space-y-3'>
						<div className='flex items-center gap-2 text-sm font-medium text-guild-muted'>
							<PrimogemIcon className='text-guild-gold' size={18} /> Current
							Banners
						</div>
						{currentBanners.map((b) => (
							<div key={b.name} className='flex items-center justify-between'>
								<span
									className={`font-medium ${ELEMENT_COLORS[b.element].text}`}
								>
									{b.name}
								</span>
					...
					{/* Live Streamers */}
					<div
						className='guild-card p-5 space-y-3 cursor-pointer hover:border-white/10 transition-colors'
					...
					>
						...
						<div className='text-3xl font-bold font-mono'>
							24{' '}
							<span className='text-sm font-normal text-guild-muted'>
								streamers
							</span>
						</div>
			...
			<section className='px-6 pb-12'>
				<div className='max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4'>
					...
					<div className='guild-card p-5 space-y-4'>
						<div className='flex items-center gap-2 text-sm font-medium text-guild-muted'>
							<VerdictIcon className='h-4 w-4 text-guild-accent' /> Trending
							Builds This Week
						</div>
						{trendingBuilds.map((b, i) => (
							<div key={b.name} className='flex items-center gap-3'>
								<span className='text-sm font-mono text-guild-muted w-4'>
									{i + 1}.
								</span>
								<span
									className={`font-medium ${ELEMENT_COLORS[b.element].text}`}
								...
								<span className='text-sm text-guild-muted ml-auto'>
									{b.uses} usage
								</span>
							</div>
						))}
			...
			<section className='px-6 pb-16'>
				<div className='max-w-5xl mx-auto space-y-4'>
					...
					{RECENT_ROASTS.map((r, i) => (
						<div
							key={i}
							className='guild-card p-4 flex items-center gap-4 hover:bg-white/5 transition-colors'
						>
							<span
								className={`font-bold text-lg ${ELEMENT_COLORS[r.element]?.text}`}
							>
								{r.uid.split('x')[0]}x
							</span>
							<div className='flex-1'>
								<p className='text-sm font-medium'>
									Roasted:{' '}
									<span
										className={ELEMENT_COLORS[r.element]?.text || 'text-white'}
									>
										{r.character}
									</span>
								</p>
			...
			<section className='px-6 pb-12'>
				<div className='max-w-3xl mx-auto guild-card p-8 text-center space-y-4 guild-glow'>
					...
					<p className='text-guild-muted max-w-md mx-auto'>
						Paste your UID above and we&apos;ll judge every artifact you own. No
						mercy. No filter. Just facts.
					</p>
				</div>
			</section>
		</div>
